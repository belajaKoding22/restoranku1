        ...
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    <h3>Daftar Menu</h3>
                    <p class="text-subtitle text-muted">Berbagi pilihan menu terbaik</p>
                </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <form action="{{ route('items.destroy', $item->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Apakah kamu yakin ingin menghapus menu ini?')">
                                    ...
                                    {{-- <td>
                                        @if ($item->is_active == 1)
                                            <form action="{{ route('items.updateStatus', $item->id)}}" method="POST">
                                                ...
                                                <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Apakah kamu yakin ingin menonaktifkan menu ini?')">
                                                    <i class="bi bi-x">Nonaktifkan</i>
                                                </button>
                                            </form>
                                        @else
                                            <form action="{{ route('items.updateStatus', $item->id) }}" method="POST">
